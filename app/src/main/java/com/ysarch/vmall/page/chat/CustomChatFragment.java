...
import com.ysarch.vmall.page.chat.chatrow.ChatRowOrderV2;
...
//        titleBar.setBackgroundColor(getResources().getColor(android.R.color.holo_red_light));
        titleBar.setLeftImageResource(R.drawable.ic_back_black);
...
//                }
        ...
        titleBar.setRightImageResource(R.drawable.ic_trash_can);
...
//    @Override
//    public void onMessageItemClick(Message message, MessageList action) {
//        switch (action) {
//            case ITEM_TO_NOTE:
    ...
    private final class DemoCustomChatRowProvider implements CustomChatRowProvider {
        ...
        public ChatRow getCustomChatRow(Message message, int position, BaseAdapter adapter) {
            if (message.getType() == Message.Type.TXT) {
                switch (MessageHelper.getMessageExtType(message)) {
                    case OrderMsg:
                        return new ChatRowOrderV2(getActivity(), message, position, adapter);
                }
            }
            return null;
        }
